...
rapid_profiler_exit_trampoline:
    ...
    movdqu xmm1, [rsp + 32]
    add rsp, 48
    .globl rapid_profiler_exit_trampoline_notrack_jump
rapid_profiler_exit_trampoline_notrack_jump:
    notrack jmp r11
...
unsafe extern "C" {
    ...
    #[cfg(test)]
    fn rapid_profiler_exit_trampoline_notrack_jump();
}
...
impl ProfilerState {
    ...
    pub(crate) fn run_epoch(
        ...
        // the profiler contract requires the documented workload ABI.
...
pub(crate) unsafe extern "C" fn count_function(context: *mut HookContext) {
    if context.is_null() {
        return;
    }
...
fn exit_trampoline_address() -> u64 {
    rapid_profiler_exit_trampoline as *const () as usize as u64
}
...
#[cfg(test)]
mod tests {
    use super::{AtomicDistribution, rapid_profiler_exit_trampoline_notrack_jump, take_sample};
    use core::sync::atomic::AtomicU64;

    const CET_NOTRACK_JMP_R11: [u8; 4] = [0x3e, 0x41, 0xff, 0xe3];

    fn is_cet_notrack_jmp_r11(bytes: &[u8]) -> bool {
        bytes == CET_NOTRACK_JMP_R11
    }

    #[test]
    fn exit_continuation_emits_cet_notrack_indirect_jump() {
        let address = rapid_profiler_exit_trampoline_notrack_jump as *const () as *const u8;
        // SAFETY: the assembly label identifies the start of the four-byte
        // NOTRACK JMP R11 instruction in the linked executable text.
        let bytes = unsafe { core::slice::from_raw_parts(address, CET_NOTRACK_JMP_R11.len()) };
        assert_eq!(bytes, CET_NOTRACK_JMP_R11);
        assert!(is_cet_notrack_jmp_r11(bytes));
    }

    #[test]
    fn plain_indirect_jump_is_not_accepted_as_cet_notrack() {
        assert!(!is_cet_notrack_jmp_r11(&CET_NOTRACK_JMP_R11[1..]));
    }
