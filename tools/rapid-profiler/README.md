...
the original return value, and jumps to the real caller address. That indirect
continuation carries CET IBT's `NOTRACK` prefix, so the caller continuation need
not begin with `ENDBR64`. A LIFO frame stack handles ordinary nesting,
...
whole sleds using its own runtime protocol. The current profiler therefore uses
one entry probe plus return-address hijacking instead of mutating every XRay
exit sled.
...
AVX-width returns. The return continuation supports CET Indirect Branch
Tracking by using `NOTRACK`; CET shadow stacks remain unsupported because
return-address hijacking changes the ordinary stack without changing the
hardware shadow stack. Exceptions, stack unwinding, `setjmp`/`longjmp`, and
workloads that leave instrumented calls running on other threads are also
unsupported because they can bypass or outlive the hijacked return slot.
...
`-falign-functions=4096` provides that property for the fixture. Installation
validates the complete five-byte patch window against the scan snapshot before
publishing the probe. This loader also owns the complete copied code image and
exposes no competing writer for that window.
