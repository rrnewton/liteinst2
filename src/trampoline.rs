...
/// Process-lifetime W^X trampoline storage prepared before instrumentation.
///
/// An arena keeps separate RW and RX aliases and reserves fixed-size slots
/// without issuing syscalls. This lets a signal-driven client prepare storage
/// before installing a seccomp filter, then emit site-specific trampolines on
/// the first trap.
...
impl InstalledHook {
    ...
    /// lifetime and must not unwind across its C ABI boundary.
    ...
    /// # Safety
    ///
    /// The same requirements as install apply. The callback must implement
